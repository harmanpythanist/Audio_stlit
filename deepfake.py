...
st.set_page_config(page_title='Audio preprocessing', page_icon=':parrot:', layout="centered", initial_sidebar_state="auto")
#-----------------------------------------------------------------------
...
d, a, b, c, e = st.tabs(['Home','Speech Recognition', 'Deep Fake Audio Detection', 'Voice Chart', 'Noise Reduction'])
...
with d:
    ...
    display_text_shape('Audio processing refers to the manipulation and analysis of audio signals using digital techniques. '
    ...
    display_text_shape("The Audio Processing Project is a comprehensive endeavor designed to explore and enhance the capabilities of audio signal "
                       ...
                       "quality and efficiency of audio processing tasks and its contribution to the broader audio technology community.", size=15)
    ...
    display_text_shape("Voice detection, also known as voice activity detection (VAD), is a crucial component in the field of audio processing. It serves as "
...
with a:
    display_underline('Speech Recognition', size=16, underline=True)
    display_underline('Say something and we will detect your voice!!', color='yellow', size=16)
    st.write('.')
    st.write('.')
    ...
    with c2:
        text=speech_to_text(stop_prompt='click here to stop recording', start_prompt='click here to start recording' , language='en', use_container_width=True,just_once=True,key='STT')
        st.write(text)
    if text:
...
with c:
    ...
    try:
        ...
        gm_ints = np.frombuffer(audio['bytes'], dtype='int16')  # bytes into integers
        ...
        ax.plot(time_stamps, gm_ints)

        ax.set_xlabel('Time')
        ax.set_ylabel('Intensity')
        # Display the Matplotlib figure using Streamlit
        st.pyplot(fig)
        #st.pyplot(time_stamps, gm_ints)
    except:
        st.warning('You voice chart will show here')
...
def classify_audio_clip(clip):
    ...
    state_dict = torch.load('D://fake audio detection/classifier.pth', map_location=torch.device('cpu'))
...
with b:
    audio = mic_recorder(start_prompt="⏺️", stop_prompt="⏹️", key='recordr')

    if audio:
        a = st.audio(audio['bytes'])
        #gm_ints = np.frombuffer(audio['bytes'], dtype='float32')
        #load_audio(a)
        #ao = torch.FloatTensor(gm_ints)
        #ao.unsqueeze(0)
        temp_file = create_temporary_wav_file(audio['bytes'], 22000)
        r = classify_audio_clip(load_audio(temp_file))
        your_answer = r.detach().numpy()
        ans = your_answer.tolist()
        st.write(ans)
        if ans < 0.50:
            display_underline('This likely does not belong to a ai robot', size=17)
        elif ans >=0.50:
            display_underline('most likely to be a AI GENERATED VOICE', size=17)

        


        
        #r = classify_audio_clip(a)
        #st.write(r)
        ...
        #torch_tensor = torch.tensor(gm_ints)
        #r = classify_audio_clip(torch_tensor)
    ...
    try:
        ...
        st.warning('You voice chart and prediction will show here')
...
with e:
    display_underline('Upload your audio with noise', size=19)
    ...
    def fun_record():
        ...
        if uploaded_file is not None:
            # Convert uploaded file to AudioFile
            audio_file = file_to_audio_file(uploaded_file)
            ...
            #st.write(recognizer.recognize_google(audio_data))

            # Adjust for ambient noise
            #with audio_file as a:
             #   recognizer.adjust_for_ambient_noise(a, duration=2.0)

            with audio_file as source:
                audio_data_with_noise_reduction = recognizer.record(source)

            # Display text after noise reduction
            st.subheader("Text After Noise Reduction:")
            st.write(recognizer.recognize_google(audio_data_with_noise_reduction))

            # Play audio with noise reduction
            #st.subheader("Play Audio with Noise Reduction:")
            temp_wav_file = audio_data_to_temp_wav(audio_data_with_noise_reduction)
